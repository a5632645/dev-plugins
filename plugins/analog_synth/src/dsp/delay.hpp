...
public:
    ...
    float mix{};
private:
    void ProcessMono(std::span<qwqdsp_psimd::Float32x4> block) noexcept {
    ...
    void ProcessPingpong(std::span<qwqdsp_psimd::Float32x4> block) noexcept {
        float delay_samples = delay_ms * fs / 1000.0f;
        delay_samples = std::max(delay_samples, 1.0f);
        float lp_coeff = lp_filter_.ComputeCoeff(qwqdsp::convert::Freq2W(lowcut_f, fs));
        float hp_coeff = hp_filter_.ComputeCoeff(qwqdsp::convert::Freq2W(highcut_f, fs));
    ...
    qwqdsp_fx::DelayLineSIMD<qwqdsp_psimd::Float32x4, qwqdsp_fx::DelayLineInterpSIMD::PCHIP> delay_;
    qwqdsp_filter::OnePoleTPTSimd<qwqdsp_psimd::Float32x4> lp_filter_;
    qwqdsp_filter::OnePoleTPTSimd<qwqdsp_psimd::Float32x4> hp_filter_;
};
}
