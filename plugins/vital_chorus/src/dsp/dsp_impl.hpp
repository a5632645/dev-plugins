...
public:
    ...
    void Update(const DspParam& p) override {
        ...
        if (num_voices_ != p.num_voice) {
            for (int i = num_voices_ / simd::LaneSize<SimdType>; i < p.num_voice / simd::LaneSize<SimdType>; ++i) {
                lowpass_[i].Reset();
                highpass_[i].Reset();
                delays_[i].Reset();
            }
    ...
    void Process(float* __restrict left_ptr, float* __restrict right_ptr, int num_samples) override {
        int const num_pairs = num_voices_ / simd::LaneSize<SimdType>;
        ...
        while (offset != num_samples) {
            ...
            for (size_t i = 0; i < num_pairs; ++i) {
                SimdType static_a = SimdType{delay1_, delay1_, delay2_, delay2_};
                ...
                SimdType delay_ms = (depth_ * kMaxModulationMs) * sin_mod + static_delay;
...
private:
    ...
    simd::Array<ParalleOnePoleTPT<inst, SimdType>, kMaxNumChorus / simd::LaneSize<SimdType>> lowpass_;
    simd::Array<ParalleOnePoleTPT<inst, SimdType>, kMaxNumChorus / simd::LaneSize<SimdType>> highpass_;
    simd::Array<SimdType, kMaxNumChorus / simd::LaneSize<SimdType>> last_delay_samples_{};
    simd::Array<pluginshared::dsp::DelayLineMultiple<inst, SimdType>, kMaxNumChorus / simd::LaneSize<SimdType>> delays_;
