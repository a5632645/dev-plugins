...
struct SSESincDelayLine {
     ...
     */
    SSESincDelayLine(const SurgeSincTableProvider& st)
        : sinctable(st.sinctable) {
        clear();
    }
    ...
    inline float read(float delay) {
        ...
        auto fracDelay = delay - static_cast<float>(iDelay);
        ...
        // hand-written SSE2: three 4-lane dot products + horizontal sum
    ...
    inline float readLinear(float delay) {
        auto iDelay = (int)delay;
        auto frac = delay - static_cast<float>(iDelay);
