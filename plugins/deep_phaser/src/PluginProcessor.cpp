                       ...
                       )
{
    ...
    // allpass lfo
    {
        auto [pfreq, ptype] = blend_lfo_state_.Build("blend", -10.0f, 10.0f, 0.01f, 0.4f, true, "-1/64T", "1/64T", 0.2f, "1", false);
    ...
    {
        auto [pfreq, ptype] = barber_lfo_state_.Build("barber", -10.0f, 10.0f, 0.01f, 0.4f, true, "-1/64T", "1/64T", 0.2f, "1", false);
        layout.add(std::move(pfreq));
        layout.add(std::move(ptype));
    }
    {
                                              ...
                                              juce::MidiBuffer& midiMessages)
{
    ...
    {
        auto barber_info = barber_lfo_state_.SyncBpm2(getPlayHead());
        if (barber_info.sync_lfo) {
            barber_oscillator_.Reset(barber_info.lfo_phase * std::numbers::pi_v<float> * 2);
        }
        auto blend_info = blend_lfo_state_.SyncBpm2(getPlayHead());
        if (blend_info.sync_lfo) {
            blend_lfo_phase_ = blend_info.lfo_phase;
        }
    }
    ...
    while (cando != 0) {
        ...
        float frac_temp;
        auto barber_info = barber_lfo_state_.SyncBpm2(getPlayHead());
        barber_oscillator_.SetFreq(barber_info.lfo_freq, fs);
        auto blend_info = blend_lfo_state_.SyncBpm2(getPlayHead());
        float const blend_lfo_phase_inc = blend_info.lfo_freq * static_cast<float>(num_process) / fs;
...
void DeepPhaserAudioProcessor::UpdateCoeff() {
    ...
    if (!is_using_custom_) {
        ...
        float const beta = qwqdsp_window::Kaiser::Beta(param_fir_side_lobe_->get());
