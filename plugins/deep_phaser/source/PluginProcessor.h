...
public:
    ...
    auto GetAfterPush(SimdIntType rpos) const noexcept {
        ...
        SimdType y0;
        y0.x[0] = buffer[irpos.x[0]].x[0];
        y0.x[1] = buffer[irpos.x[1]].x[0];
        y0.x[2] = buffer[irpos.x[2]].x[0];
        y0.x[3] = buffer[irpos.x[3]].x[0];
        SimdType y1;
        y1.x[0] = buffer[irpos.x[0]].x[1];
        y1.x[1] = buffer[irpos.x[1]].x[1];
        y1.x[2] = buffer[irpos.x[2]].x[1];
        y1.x[3] = buffer[irpos.x[3]].x[1];
...
public:
    ...
    qwqdsp_filter::OnePoleTPTSimd<SimdType> damp_;
    float damp_lowpass_coeff_{1.0f};
    float last_damp_lowpass_coeff_{1.0f};

    // barberpole
    qwqdsp_filter::StereoIIRHilbertDeeperCpx<SimdType> hilbert_complex_;
