...
public:
    ...
    void Init(int block_size, int hop_size, std::span<const float> synthesis_window) {
        ...
        synthsis_window_.assign(synthesis_window.begin(), synthesis_window.end());
        Reset();
    }
            ...
            if (num_input_ >= block_size_) {
                ...
                for (int i = 0; i < block_size_; ++i) {
                    output_buffer_[static_cast<size_t>(i + write_add_begin_)] +=
                        out_frame[static_cast<size_t>(i)] * synthsis_window_[static_cast<size_t>(i)];
                }
...
private:
    ...
    std::vector<float> synthsis_window_{};
    int block_size_{};
    int hop_size_{};
    int num_input_{};
    int write_end_{};
