...
void BoxBurgLPC::Init(float fs) {
    SetBlockSize(1024);
}

void BoxBurgLPC::SetBlockSize(size_t size) {
    fft_size_ = size;
    fft_.init(size);
...
void BoxBurgLPC::Process(
    ...
    while (numInput_ >= fft_size_) {
        ...
        for (size_t i = 0; i < side.size(); ++i) {
            gain_side += side[i] * side[i];
        }
        gain_side = qwqdsp_simd_element::PackOps::Sqrt(gain_side);
        auto atten = gain / (gain_side + 1e-8f);
        // iir lattice
...
void BoxBurgLPC::CopyLatticeCoeffient(std::span<float> buffer) {
