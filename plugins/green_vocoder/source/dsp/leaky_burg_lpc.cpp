...
void LeakyBurgLPC::ProcessWithDicimate(
    ...
    if constexpr (kDicimate == 1) {
        ...
        for (size_t i = 0; i < dicimate_counter_; ++i) {
            main[i] = upsample_latch_;
        }
        size_t idx = dicimate_counter_;
        for (;idx < main.size(); idx += kDicimate) {
            // adding some noise to prevent ill filter coefficient
            float small_noise = noise_.Next() * kNoiseGain;
            qwqdsp_simd_element::PackFloat<2> main_x = main[idx];
            main_x += small_noise;
            // forward fir lattice
            qwqdsp_simd_element::PackFloat<2> ef = main_x;
            qwqdsp_simd_element::PackFloat<2> eb = main_x;
            ...
            // upsample
            for (int j = 0; j < kDicimate; ++j) {
                main[idx + j] = upsample_latch_;
            }
        }
        dicimate_counter_ = idx - main.size();
        // we still have some not processing
        if (idx > main.size()) {
            idx -= kDicimate;
            // adding some noise to prevent ill filter coefficient
            float small_noise = noise_.Next() * kNoiseGain;
            ...
            upsample_latch_ = x_iir_[lpc_order_];
            // upsample
            for (size_t j = idx; j < main.size(); ++j) {
                main[j] = upsample_latch_;
            }
        }
    }
}
