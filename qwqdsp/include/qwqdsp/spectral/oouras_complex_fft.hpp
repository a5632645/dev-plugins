 ...
 */
class OourasComplexFFT {
public:
    /**
     * @param fft_size 必须是2^N
     */
    ...
    /**
     * @param input_real size()=fft_size
     * @param input_imag size()=fft_size
     * @param output_real size()=fft_size
     * @param output_imag size()=fft_size
     */
    void FFT(
        const float* input_real, const float* input_imag,
        float* outupt_real, float* output_imag
    ) noexcept {
        for (size_t i = 0; i < fft_size_; ++i) {
            buffer_[2 * i] = input_real[i];
            buffer_[2 * i + 1] = input_imag[i];
        }
        cdft(fft_size_ * 2, 1, buffer_.data(), ip_.data(), w_.data());
        outupt_real[0] = buffer_[0];
        output_imag[0] = buffer_[1];
        for (size_t i = 1; i < fft_size_; ++i) {
            outupt_real[fft_size_ - i] = buffer_[i * 2];
            output_imag[fft_size_ - i] = buffer_[i * 2 + 1];
        }
    }
    ...
    void IFFT(
        ...
        buffer_[1] = in_imag[0];
        for (size_t i = 1; i < fft_size_; ++i) {
            buffer_[2 * i] = in_real[fft_size_ - i];
            buffer_[2 * i + 1] = in_imag[fft_size_ - i];
        }
        cdft(fft_size_ * 2, -1, buffer_.data(), ip_.data(), w_.data());
        const float gain = 1.0f / fft_size_;
        for (size_t i = 0; i < fft_size_; ++i) {
            out_real[i] = buffer_[i * 2] * gain;
            out_imag[i] = buffer_[i * 2 + 1] * gain;
        }
    }
private:
